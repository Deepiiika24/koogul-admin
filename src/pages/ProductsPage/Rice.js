...
function Rice() {
  ...
  const handleInputChange = (e) => {
    ...
    setRiceForm(prev => ({ ...prev, [name]: value }));
  };

  const handleImageChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      setRiceForm(prev => ({ ...prev, image: file }));
  ...
  const handleOpenMenu = (event, rice) => {
  ...
  const handleOpenEditDialog = () => {
    setIsEditMode(true);
    setRiceForm({
      name: selectedRice.name,
      price: selectedRice.price,
      image: null
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('price', riceForm.price);
      if (riceForm.image) {
        formData.append('image', riceForm.image);
      }

      if (isEditMode && selectedRice) {
        await putData(`rice/${selectedRice.id}`, formData, token, true);
      } else {
        await postData('rice', formData, token, true);
      }

      handleClose();
      toast.success("Rice Added Successfully!")
      getAllRices(currentPage, limit);
    } catch (err) {
      console.error('Error submitting Rice:', err);
      toast.error("Error submitting Rice:", err);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      if (selectedRice) {
        await deleteData(`rice/${selectedRice.id}`, token);
        getAllRices(currentPage, limit);
      }
      handleCloseMenu();
      toast.success("Rice Deleted Successfully!");
    } catch (err) {
      console.error('Error deleting Rice:', err);
      toast.error('Error deleting Rice:', err)
    }
  };

  const getAllRices = async (page = 1, limit = 5) => {
    try {
      const params = { page, limit };
      const res = await getData('rice', params);
      setRices(res.data);
          ...
          <Dialog open={open} onClose={() => { handleClose(); handleCloseMenu(); }} fullWidth maxWidth="sm">
            ...
            <DialogContent>
              ...
              <input
                accept="image/*"
                style={{ display: 'none' }}
                id="rice-image-upload"
                type="file"
                onChange={handleImageChange}
              />
              <label htmlFor="rice-image-upload">
                <Button
                  variant="outlined"
                  component="span"
                  fullWidth
